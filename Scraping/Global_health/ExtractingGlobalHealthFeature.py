...
class GlobalHealthExtractor:
    def __init__(self,news_data,file_path,health_terms):
    ...
    def global_health_crisis(self,threshold):
        ...
        monthly_df.to_csv("Scraping\\Global_health\\globalCrisisFeature.csv",index=False)
...
if __name__=="__main__":
    with open('Scraping\\Global_health\\GlobalHealthNews.json',"r") as f:
        news_data = json.load(f)
    health_terms = ['emergency','pandemic', 'virus', 'outbreak', 'covid', 'coronavirus','disease']
    file_path  = "Scraping\\Global_health\\GlobalHealthFeature.csv"
    ghe = GlobalHealthExtractor(news_data,file_path,health_terms)
    ghe.run()